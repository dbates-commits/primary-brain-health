...
8. [Team](#8-team)
9. [Appendix - Diligence Reference (RFP Section 8 Responses)](#9-appendix---diligence-reference-rfp-section-8-responses)
...
The V1 site-cleanup punchlist (RFP V1 defects P1–P8) is **closed and verified on staging by Jun 12, 2026** as a named acceptance criterion.
...
| N4 | **Eisai transitional landing page + co-branded results variant** *(new)* | (1) Linus-branded transitional landing page receiving traffic from `leqembi.com` / `eisaipatientsupport.com`; sponsor-mark visible (not headline co-branding); HIPAA consent + Eisai sponsor-specific consent variant; sets `cohort=eisai-sponsored` flag at signup; no Stripe step. (2) Co-branded results-page variant for the Eisai cohort - same layout, same data, sponsor mark visible, sponsor-appropriate disclaimer language. **Cross-engagement billing (per PBH, Jun 3):** a portion of N4 (the dedicated Eisai surfaces) bills to the **updated Eisai SOW #14 by default**, contingent on cross-engagement decision #4 landing as funnel-in. Sponsor-mark branding follows the same constraint (visible, not headline co-branding) regardless of which engagement bills it. **Fallback:** if the SOW #14 is not updated, the amount reverts to PBH's Phase 2 envelope at the same milestone schedule (see Phase 2 pricing). |
...
| C3 | **Pricing / Membership page** | Standalone `/pricing` page improves SEO and reduces friction. What's included, HSA/FSA language, future membership tiers. **In-scope:** static design and layout. **Named change order (approved Jun 25, 2026):** two additions beyond the static page — (1) TinaCMS-driven price/tier editing so pricing can be updated without a code release, and (2) step-to-step conversion instrumentation through the acquisition arc. Estimated 28–30 hours (see change order for pricing). Proposed as an immediate fast follow after Phase 1 launch to avoid jeopardizing the Aug 14 go-live. |
...
**Optional add-on**: External penetration test on payment + auth surfaces (separately scoped, pricing on request). Recommended for additional defense in depth but not in core scope.
...
This estimate includes **2 rounds of revisions per surface** (Figma mockup or staging implementation). Additional revisions beyond the 2 rounds are billed at our standard hourly rate and tracked as line items in the next milestone invoice.
...
- **External penetration test** - flagged as a recommended optional add-on (separately scoped via a security firm). Our subcontracted HIPAA/PCI specialist provides architecture review + pre-launch sign-off, but adversarial pen-testing is a separately scoped engagement
...
> **⚠️ NOT included in the core engagement scope.** Optional content-production services (founder video, patient testimonials, hero loop additions, resource-bundle design, photography, email-sequence copywriting) are PBH-elected line items priced in §9. Firm quotes already received for several lines; the rest remain indicative pending sourcing. PBH can elect or decline any item independently with no impact on engagement scope or price.
...
## 9. Appendix - Diligence Reference (RFP Section 8 Responses)
...
- **Can you deliver video testimonial production in-house, or do you partner?** We partner with a videographer rather than producing in-house, coordinated under VisualBoston so PBH has a single point of contact. Production is scoped as an optional line item (additional cost, quoted separately).

- **What does ongoing monthly support look like post-launch?** Two retainer tiers are available: a lighter tier (20 hrs/mo) covering CMS support, dependency updates, security patches, and monitoring incident response; and a fuller tier (40 hrs/mo) adding light feature work, content updates beyond CMS reach, and quarterly architectural review. Out-of-retainer work billed at our standard hourly rate. Pricing available on request. Starts Aug 19, 2026, or rolls together with Phases 2 and 3.
