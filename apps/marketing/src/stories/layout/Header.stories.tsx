...
const meta = {
  title: 'Layout/Header',
  component: Header,
  // In the app this comes from `AUTH0_ENABLED`, which is server-only. Without
  // it the header renders no Login control at all and every story below that
  // looks for one fails.
  args: { auth0Enabled: true },
  parameters: {
    layout: 'fullscreen',
    ...
    docs: {
      description: {
        component:
          'The site header, in both session states. Signed out it carries FAQs, Contact, a ' +
          'Login control that hands off to Auth0, and the Book a Consultation CTA. ' +
          'Signed in it matches Figma ' +
