...
export function BenefitsList({
  ...
  const getItemField = (index: number, field: string) =>
    blockData?.items?.[index]
      ...
      : undefined;
  return (
    <section className="bg-background-teal-subtle px-6 md:px-10 pt-20 md:pt-28 pb-20 md:pb-28">
      <div className="max-w-6xl mx-auto flex flex-col gap-10 md:gap-14">
        ...
        >
          {items.map((item, i) => (
            ...
            >
              {item.icon && (
                <div className="flex items-center justify-center shrink-0 w-16 h-16 md:w-20 md:h-20 rounded-full bg-background-default shadow-[0_8px_18px_-12px_rgba(0,0,0,0.18)]">
                  <PhosphorIcon
                    name={item.icon}
                    aria-hidden="true"
                    weight="regular"
              ...
              <div className="flex flex-col items-center">
                {item.title && (
                  <h3
                    ...
                    className="font-headline font-thin text-grey-850 text-2xl md:text-3xl leading-[1.2] text-balance"
                  >
                    {item.title}
                  </h3>
                )}
