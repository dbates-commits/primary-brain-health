...
export const linusEnrollments = pgTable(
  "linus_enrollments",
  {
     ...
     *
     * This is the one part of the track axis that is stored rather than derived
     * from `payments.package_key`: entitlement is a fact about the user *now*,
     * while this is a fact about a piece of work already done, and a later
     * purchase must not rewrite it. Written on insert only — see
     * `upsertEnrollmentRow`.
     *
     * Defaulted because every enrollment that predates the clinical track was a
     * wellness one.
     */
